...
- **検証可能なゲート**: QA、デプロイ、最終チェックは実際に実行されるコマンドであり、その判定はイベントとして記録されます。ステージが完了とされたのにゲートが不合格の場合、`boss gate final` と `boss doctor` は失敗と判定します。ゲートの実行はオーケストレータがプロトコルを守ることに依存します。CLI が提供するのは検証可能な判定と記録であり、迂回を阻止する仕組みではありません。
