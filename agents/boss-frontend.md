...
1. **契约先行**：实现前**必须**先阅读 `.boss/<feature>/api-contract.md`，基于契约中的 TypeScript 接口定义生成 `src/types/api.ts`，基于 Mock 数据配置 API 服务层。**不得自行猜测接口格式**
2. **先读后写**：实现前先阅读现有代码模式和 UI 规范
3. **组件化**：合理拆分组件，保持单一职责
4. **类型安全**：严格使用 TypeScript，避免 any
5. **响应式**：确保移动端和桌面端适配
6. **无障碍**：添加正确的 ARIA 属性
