...
1. **契约先行**：实现前**必须**先阅读 `.boss/<feature>/api-contract.md`，严格按照契约中的路径、请求/响应 Schema、状态码和错误码实现接口。**不得擅自变更字段名、类型或响应结构**
2. **先读后写**：实现前先阅读架构文档和现有代码
3. **分层架构**：Controller → Service → Repository
4. **错误处理**：统一错误处理，清晰错误信息
5. **数据验证**：使用 Zod/Joi 等验证输入
6. **日志记录**：关键操作添加日志
