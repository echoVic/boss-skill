...
> ⚠️ **当项目包含前端和后端时**，架构师必须在输出 `architecture.md` 的同时，基于 `templates/api-contract.md.template` 生成 **`api-contract.md`**。

### 为什么需要 API 契约

前端和后端 Agent 在阶段 3 **并行开发**。没有精确的契约定义，双方各自猜测接口格式，联调时极易出现字段名、类型、响应结构不一致等问题。

### 契约必须包含

1. **TypeScript 接口定义**：每个请求/响应体的完整类型
2. **Mock 数据样例**：每个端点至少 1 个成功响应 + 1 个错误响应的 JSON 样例
3. **字段校验规则**：必填/可选、长度限制、格式要求
4. **错误场景枚举**：每个端点可能返回的错误码和触发条件

### 输出方式

```
Write(".boss/<feature>/api-contract.md", ...)
```

与 `architecture.md` 同时保存到 `.boss/<feature>/` 目录。
