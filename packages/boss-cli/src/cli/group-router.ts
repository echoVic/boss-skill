...
import { commandDescriptions, runtimeCommandDescriptions, runtimeCommandNames } from './command-registry.js';
import {
  artifactDescription,
  hooksDescription,
  packsDescription,
...
import { loadRuntimeCommand } from './runtime-loader.js';
...
export async function runRuntimeCommand(argv: string[]): Promise<number> {
  ...
  const load = loadRuntimeCommand(runtimeCommand);
  if (!load) {
    throwUnknownCommand('boss runtime', runtimeCommand);
  }
