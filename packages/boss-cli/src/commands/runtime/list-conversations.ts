...
export function main(argv: string[] = process.argv.slice(2), { cwd = process.cwd() }: { cwd?: string } = {}): number {
  ...
  try {
    ...
    writeOutput(payload, context, (data) => renderConversationListText(data as Array<Record<string, unknown>>));
    return 0;
  } catch (err) {
...
if (process.argv[1] && path.resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  const context = createCliContext(process.argv.slice(2), { command: 'boss runtime list-conversations' });
